...
def grayscale(image_counter, unique_image_name, images_dir, img_np, labels_dir, labels, classes, cat_image):
    # Define the name and path for the grayscale image
    gray_image_name = f"gray_{image_counter}_{unique_image_name}"
    gray_image_path = os.path.join(images_dir, gray_image_name)

    # Convert the image to grayscale
    img_gray = cv2.cvtColor(img_np, cv2.COLOR_BGR2GRAY)
    cv2.imwrite(gray_image_path, img_gray)

    # Save the same labels for the grayscale image, adjusting the bounding boxes
    gray_label_path = os.path.join(labels_dir, f'{os.path.splitext(gray_image_name)[0]}.txt')
    with open(gray_label_path, 'w') as f:
        for label in labels:
            ...
            height = height * 640  # Resize the height to 640x64
