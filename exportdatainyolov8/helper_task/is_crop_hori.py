...
def crop_hor(image_counter, unique_image_name, images_dir, img_np, labels_dir, labels, classes, cat_image):
    """
    Black out the top half of all bounding box areas in the same image and adjust the bounding box dimensions.
    ...
    for label in labels:
        ...
        # Determine the region to black out (top half)
        black_x_min = x_min
        black_x_max = x_max
        black_y_min = y_min
        black_y_max = y_min + (y_max - y_min) // 2  # Top half

        # Apply the blackout
        modified_img[black_y_min:black_y_max, black_x_min:black_x_max] = 0

        # Adjust the bounding box to the remaining visible half (bottom half)
        new_x = x_min
        new_y = black_y_max
        new_width = x_max - x_min
        new_height = y_max - black_y_max
    ...
    modified_image_name = f"blackout_hor_{image_counter}_{unique_image_name}.jpg"
    ...
    logger.info(f'Processed image {unique_image_name} with all bounding boxes blacked out (top half).')
