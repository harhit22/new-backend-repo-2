    ...
    def get(self, request, project_id, category_id):
        ...
        try:
            with transaction.atomic():
                # First check for an "in_progress" image assigned to the user
                status_instance = CategoryImageStatus.objects.select_for_update().filter(
                    category=category,
                    status='in_progress',
                ...
                if status_instance is None:
                    # Check for an "unlabeled" image not yet assigned
                    status_instance = CategoryImageStatus.objects.select_for_update().filter(
                        category=category,
                        status='unlabeled',
                ...
                if status_instance is None:
                    # Fetch an unassigned image and assign it
                    unassigned_image = Image.objects.select_for_update().filter(
                        project_id=project_id
                    ).exclude(
                        category_statuses__category=category,
                        category_statuses__assigned_to__isnull=False
                    ).exclude(original_image__status='in_progress').order_by('uploaded_at').first()

                    if unassigned_image is None:
                        return Response({"error": "No more images available"}, status=status.HTTP_403_FORBIDDEN)

                    if CategoryImageStatus.objects.filter(category=category, image=unassigned_image,
                    ...
                    # Assign the fetched status instance to the user
...
class DeleteAlreadyLabelCategoryImage(APIView):
    ...
    def delete(self, request, category_id):
        ...
        print(f"Deleted {deleted_count} labels for category_image_id: {category_id}")
        return JsonResponse({"success": True, "message": f"{deleted_count} labels deleted successfully"})



@method_decorator(csrf_exempt, name='dispatch')  # For testing purposes; remove in production
class UpdateCategoryLabel(APIView):
    """
    Class-based view for updating or creating category labels.
    """
    ...
    def post(self, request):
        try:
            ...
            label_id = data.get("label_id")  # Check if label_id exists
            print(label_id)
            ...
            # If label_id exists, update the label; otherwise, create a new one
            if label_id:
                try:
                    label = ImageLabel.objects.get(label_id=label_id)
            ...
            return Response(
                {"message": "Label saved successfully", "label_id": label_id},
